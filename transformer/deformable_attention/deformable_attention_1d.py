...
class CPB(nn.Module):
    ...
    def __init__(self, dim, *, heads, offset_groups, depth, log_distance=True):
        ...
        self.mlp.append(nn.Sequential(nn.Linear(1, dim), nn.ReLU()))
...
class DeformableAttention1D(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        # if offset_scale is none offset_scale -> downsample_factor
        ...
        assert offset_kernel_size >= downsample_factor

        # offset kernel size - downsample factor must be divisible by 2
        assert divisible_by(offset_kernel_size - downsample_factor, 2)

        # if offset_groups is none offset_groups -> heads
        offset_groups = default(offset_groups, heads)

        # heads must be divisible by offset groups
        assert divisible_by(heads, offset_groups)

        inner_dim = dim_head * heads  # hidden_dim
        ...
        self.to_offsets = nn.Sequential(
            ...
            nn.GELU(),
            # pointwise convolution : information exchange between channels
        ...
        # log-space continuous positional bias
        self.rel_pos_bias = CPB(
            dim // 4,
            offset_groups=offset_groups,
            heads=heads,
            depth=2,
            log_distance=cpb_log_distance,
        )

        self.dropout = nn.Dropout(dropout)

        # linear projection layers x->q,k,v
        self.to_q = nn.Conv1d(
    ...
    def forward(self, x, return_vgrid=False):
        ...
        heads, b, n, downsample_factor, device = (
            self.heads,
            x.shape[0],
            x.shape[-1],
        ...
        q, k, v = map(
            lambda t: rearrange(t, "b (h d) n -> b h n d", h=heads), (q, k, v)
        )
